...
function SectionTitle({ children }) {
  return (
    <div className="inline-flex items-center bg-primary/5 rounded-lg px-3 py-1.5 mb-4">
      <p className="text-xs text-primary uppercase tracking-widest font-light">{children}</p>
    </div>
  )
}

function HelperText({ children }) {
  return <p className="text-xs text-body/60 font-light mt-0.5 leading-relaxed">{children}</p>
}
...
function SaveToast({ visible }) {
  ...
  return (
    <div className="fixed bottom-24 left-1/2 -translate-x-1/2 z-50 bg-primary text-white rounded-full px-4 py-2 text-sm font-light shadow-stripe-sm pointer-events-none">
      ✓ Saved
    </div>
  )
}
      ...
      <div className="px-4 pt-4 pb-12">
          ...
          <div className="space-y-4">
            <div>
              ...
              <input
                ...
                className="w-full border border-border-default rounded-lg px-3 py-2 text-sm font-light focus:outline-none focus:border-primary"
              />
              <HelperText>Helps adjust predictions for age-related cycle changes</HelperText>
            </div>
            ...
            <HelperText>Used to flag BMI-related cycle variability in predictions</HelperText>
            <div>
              ...
              <select
                ...
                className="w-full border border-border-default rounded-lg px-3 py-2 text-sm font-light focus:outline-none focus:border-primary bg-white"
              >
                {CONTRACEPTIVES.map(c => <option key={c}>{c}</option>)}
              </select>
              <HelperText>Hormonal contraceptives affect prediction accuracy — shown as a notice on your dashboard</HelperText>
            </div>
            <div>
                ...
                {CONDITIONS.map(cond => {
                  ...
                  return (
                    <button
                      ...
                      className={`px-3 py-1 rounded-lg text-xs font-light border transition-colors ${
              ...
              <HelperText>These widen the prediction confidence interval to account for cycle variability</HelperText>
            </div>
            <div>
              <div className="flex items-center justify-between mb-1">
                <div>
                  <label className="text-xs text-label font-light">Smart cycle length</label>
                  <HelperText>Auto-calculated from your logged cycles</HelperText>
                </div>
                <Toggle
              ...
              {p.smartCycleLength === false && (
                <div className="mt-2">
                  ...
                  <input
                    ...
                    className="w-full border border-border-default rounded-lg px-3 py-2 text-sm font-light focus:outline-none focus:border-primary"
                  />
                  <HelperText>Used when you know your cycle better than your history suggests</HelperText>
                </div>
              )}
            </div>
            <div className="flex items-center justify-between">
              <div>
                ...
                <HelperText>Pauses cycle predictions while active</HelperText>
              </div>
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <HelperText>Replaces &quot;period&quot; with &quot;cycle&quot; everywhere in the app</HelperText>
              </div>
              ...
              <div className="flex gap-1">
                {['C', 'F'].map(u => (
                  <button
                    ...
                    className={`px-3 py-1 text-xs font-light rounded-lg border transition-colors ${
        ...
        <Card className="p-4 mb-4">
          ...
          <div className="space-y-5">
            <div>
              <p className="text-sm text-heading font-light mb-1">App PIN lock</p>
              <HelperText>Hashed with PBKDF2 — raw PIN never stored on this device</HelperText>
              <div className="flex gap-2 mt-2">
                <input
                  type="password" inputMode="numeric" maxLength={4}
                  value={pinInput} onChange={e => setPinInput(e.target.value)}
                  placeholder="4-digit PIN"
                  className="flex-1 border border-border-default rounded-lg px-3 py-2 text-sm font-light focus:outline-none focus:border-primary"
                />
                <Button onClick={handleSetPin} size="sm">Set PIN</Button>
              </div>
              {p.pinHash && (
                <button onClick={handleRemovePin} className="text-xs text-primary font-light mt-1.5 hover:underline">
                  Remove PIN
                </button>
              )}
            ...
            <div>
              ...
              <HelperText>Download all your data as JSON — useful for healthcare provider visits</HelperText>
              <div className="mt-2">
        ...
        <div className="border-t-2 border-rose-100 pt-6 mt-2">
          <Card className="p-4">
            <SectionTitle>Danger Zone</SectionTitle>
            <div>
              <p className="text-sm text-heading font-light mb-1">Delete all data</p>
              <HelperText>Permanently wipes all cycles, symptoms, and settings from this device. Cannot be undone.</HelperText>
              <div className="mt-3">
